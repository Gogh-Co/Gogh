...
for theme in themes:
    ...
    while filename.exists():
        filename = output_dir / f'{name}-{counter}.yaml'
        counter += 1
    # Write the theme dictionary in the required order to YAML file
    ordered_theme = {
        "name": theme.get("name", ""),
    ...
    with open(filename, 'w') as f:
        yaml.safe_dump(ordered_theme, f, sort_keys=False, allow_unicode=True)

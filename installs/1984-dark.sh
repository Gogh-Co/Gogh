#!/usr/bin/env bash

export PROFILE_NAME="1984 Dark"

export COLOR_01="#000000"           # Black (Host)
export COLOR_02="#FF16B0"           # Red (Syntax string)
export COLOR_03="#B3F361"           # Green (Command)
export COLOR_04="#FFEA16"           # Yellow (Command second)
export COLOR_05="#46BDFF"           # Blue (Path)
export COLOR_06="#F806FA"           # Magenta (Syntax var)
export COLOR_07="#59E1E3"           # Cyan (Prompt)
export COLOR_08="#FFFFFF"           # White

export COLOR_09="#555753"           # Bright Black
export COLOR_10="#FF16B0"           # Bright Red (Command error)
export COLOR_11="#B3F361"           # Bright Green (Exec)
export COLOR_12="#FFEA16"           # Bright Yellow
export COLOR_13="#46BDFF"           # Bright Blue (Folder)
export COLOR_14="#F806FA"           # Bright Magenta
export COLOR_15="#6BE4E6"           # Bright Cyan
export COLOR_16="#FFFFFF"           # Bright White

export BACKGROUND_COLOR="#0D0F31"   # Background
export FOREGROUND_COLOR="#FFFFFF"   # Foreground (Text)

export CURSOR_COLOR="#FFFFFF" # Cursor

apply_theme() {
    if [[ -e "${GOGH_APPLY_SCRIPT}" ]]; then
      bash "${GOGH_APPLY_SCRIPT}"
    elif [[ -e "${PARENT_PATH}/apply-colors.sh" ]]; then
      bash "${PARENT_PATH}/apply-colors.sh"
    elif [[ -e "${SCRIPT_PATH}/apply-colors.sh" ]]; then
      bash "${SCRIPT_PATH}/apply-colors.sh"
    else
      printf '\n%s\n' "Error: Couldn't find apply-colors.sh" 1>&2
      exit 1
    fi
}

# | ===========================================================================
# | Apply Colors
# | ===========================================================================
SCRIPT_PATH="${SCRIPT_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
PARENT_PATH="$(dirname "${SCRIPT_PATH}")"

if [ -z "${GOGH_NONINTERACTIVE+no}" ]; then
    apply_theme
else
    apply_theme 1>/dev/null
fi

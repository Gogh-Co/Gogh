#!/usr/bin/env bash

export PROFILE_NAME="SilkCircuit Glow"

export COLOR_01="#000000"           # Black (Host)
export COLOR_02="#FF2244"           # Red (Syntax string)
export COLOR_03="#00FF00"           # Green (Command)
export COLOR_04="#FFFF00"           # Yellow (Command second)
export COLOR_05="#0099FF"           # Blue (Path)
export COLOR_06="#FF00FF"           # Magenta (Syntax var)
export COLOR_07="#00FFFF"           # Cyan (Prompt)
export COLOR_08="#FFFFFF"           # White

export COLOR_09="#818181"           # Bright Black
export COLOR_10="#FF6666"           # Bright Red (Command error)
export COLOR_11="#66FF66"           # Bright Green (Exec)
export COLOR_12="#FFFF66"           # Bright Yellow
export COLOR_13="#66CCFF"           # Bright Blue (Folder)
export COLOR_14="#FF66FF"           # Bright Magenta
export COLOR_15="#66FFFF"           # Bright Cyan
export COLOR_16="#FFFFFF"           # Bright White

export BACKGROUND_COLOR="#0A0816"   # Background
export FOREGROUND_COLOR="#FFFFFF"   # Foreground (Text)

export CURSOR_COLOR="#00FFFF" # Cursor

apply_theme() {
    if [[ -e "${GOGH_APPLY_SCRIPT}" ]]; then
      bash "${GOGH_APPLY_SCRIPT}"
    elif [[ -e "${PARENT_PATH}/apply-colors.sh" ]]; then
      bash "${PARENT_PATH}/apply-colors.sh"
    elif [[ -e "${SCRIPT_PATH}/apply-colors.sh" ]]; then
      bash "${SCRIPT_PATH}/apply-colors.sh"
    else
      printf '\n%s\n' "Error: Couldn't find apply-colors.sh" 1>&2
      exit 1
    fi
}

# | ===========================================================================
# | Apply Colors
# | ===========================================================================
SCRIPT_PATH="${SCRIPT_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
PARENT_PATH="$(dirname "${SCRIPT_PATH}")"

if [ -z "${GOGH_NONINTERACTIVE+no}" ]; then
    apply_theme
else
    apply_theme 1>/dev/null
fi

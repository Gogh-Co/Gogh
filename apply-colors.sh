  ...
  ghostty )
    ...
    if [[ -z "${GHOSTTY_CONFIG_DIRECTORY:-}" ]]; then
      ...
      if [[ ! -d "${GHOSTTY_CONFIG_DIRECTORY}" ]] &&
         [[ -d "${HOME}/Library/Application Support/com.mitchellh.ghostty" ]]; then
        GHOSTTY_CONFIG_DIRECTORY="${HOME}/Library/Application Support/com.mitchellh.ghostty"
      fi
    fi

    CFGFILE=""
    for candidate in "config.ghostty" "config"; do
      if [[ -f "${GHOSTTY_CONFIG_DIRECTORY}/${candidate}" ]]; then
        CFGFILE="${GHOSTTY_CONFIG_DIRECTORY}/${candidate}"
        break
      fi
    done
    unset candidate
...
apply_ghostty() {
  ...
  INCLUDE_LINE="config-file = ?auto/theme.ghostty"

  prints "Installing Ghostty theme (${THEME_FILE})..."

  mkdir -p "${THEME_DIR}"

  {
    echo "# Color theme: $THEME_NAME"
    echo "# Auto-generated by Gogh (https://Gogh-Co.github.io/Gogh/)"
    echo ""
    echo "palette = 0=$COLOR_01"
    echo "palette = 1=$COLOR_02"
    echo "palette = 2=$COLOR_03"
    echo "palette = 3=$COLOR_04"
    echo "palette = 4=$COLOR_05"
    echo "palette = 5=$COLOR_06"
    echo "palette = 6=$COLOR_07"
    echo "palette = 7=$COLOR_08"
    echo "palette = 8=$COLOR_09"
    echo "palette = 9=$COLOR_10"
    echo "palette = 10=$COLOR_11"
    echo "palette = 11=$COLOR_12"
    echo "palette = 12=$COLOR_13"
    echo "palette = 13=$COLOR_14"
    echo "palette = 14=$COLOR_15"
    echo "palette = 15=$COLOR_16"
    echo ""
    echo "background = $BACKGROUND_COLOR"
    echo "foreground = $FOREGROUND_COLOR"
    echo "cursor-color = $CURSOR_COLOR"

    [[ -n "${HIGHLIGHT_BG_COLOR:-}" ]] && echo "selection-background = $HIGHLIGHT_BG_COLOR"
    [[ -n "${HIGHLIGHT_FG_COLOR:-}" ]] && echo "selection-foreground = $HIGHLIGHT_FG_COLOR"
  } > "${THEME_FILE}"
  ...
  prints "Done - \"${THEME_NAME}\" is now registered with Ghostty"
  prints ""
  prints "To use it, either pick it from Ghostty's own theme picker:"
  prints "    ghostty +list-themes        (press w to save your choice, then reload)"
  prints "or set it directly in ${CFGFILE}:"
